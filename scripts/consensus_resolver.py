...
# 天井であることの指標（項目=天井のとき、値の文節にこれが要る＝ただの数字を拾わない）
CEILING_INDICATORS = ["天井", "到達", "放出", "最大", "上限", "まで", "ハマり",
                      "スルー", "でストップ", "短縮", "でやめ", "でヤメ"]
...
SHORT_MARKERS = ["reg後", "reg時", "big後", "bb後", "at後", "cz後", "後は",
                 "引き継ぎ", "有利区間リセット"]
...
# 「800Gに短縮」「800Gまで短縮」は結果値なのでOK＝直前に に/へ/まで があるかで区別。
_AMOUNT_RE = re.compile(r"(?<![にへまで])短縮")
...
def _find_value_positions(nq: str, value) -> list[int]:
    """値の全出現位置（数字境界つき＝1000が10000/1000の一部に誤一致しない）。"""
    positions = []
...
def check_claim_identity(item, mode, scope, value, unit, quote,
    ...
    for pos in positions:
        ...
        after = nq[pos: min(len(nq), pos + vlen + 8)]  # 値の直後（否定・短縮量の隣接判定）

        # ★否定★「1500Gではなく」→この出現は打ち消し＝矛盾
        if _hit(after, NEGATION_MARKERS):
            saw_contradiction = True
            reject_reason = f"否定文脈（{after}）＝値が打ち消されている"
            continue
        # ★短縮"量"★「300G短縮」（に/へ/までが無い短縮）＝天井値でなく短縮幅
        if _AMOUNT_RE.search(after):
            saw_contradiction = True
            reject_reason = "『N G短縮』＝短縮量であって天井値でない"
            continue
