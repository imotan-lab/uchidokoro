...
def build_jsonld(machine: dict, canonical_url: str, title: str, desc: str) -> str:
    ...
    # 日付は決定論に：release_dateがある機種のみdatePublished（未来日は当日にクランプ）。
    # dateModifiedは毎日の全機種再ビルドで日付が動き信頼性が下がるため出力しない（schema.org上任意）
    release = machine.get("release_date")
    if release:
        today = datetime.date.today().isoformat()
        article["datePublished"] = f"{min(str(release), today)}T00:00:00+09:00"
    article.update({
