...
MUTATIONS = [
    ...
    },
    {
        ...
        "file": "scripts/decide_now.py",
        "before": "NUM_PATTERN = (r\"(?<![\\d.])[-−▲△+＋][ \\u3000\\t]*\\d+(?:\\.\\d+)?\"",
        "after": "NUM_PATTERN = (r\"(?!x)x\"",
        "run": ["scripts/decide_now.py"],
    },
    {
        "why": "★符号のあとの空白を読まない（★「- 500枚」の符号が落ちて、出どころの検査に入らない★）★",
        "file": "scripts/decide_now.py",
        "before": "NUM_PATTERN = (r\"(?<![\\d.])[-−▲△+＋][ \\u3000\\t]*\\d+(?:\\.\\d+)?\"",
        "after": "NUM_PATTERN = (r\"(?<![\\d.])[-−▲△+＋]\\d+(?:\\.\\d+)?\"",
        "run": ["scripts/decide_now.py"],
    },
    # ─── 2026-09-25・Codex185の指摘（守りの厚み）───
    {
        "why": "★同じ欄が modeData と直下にあるとき、片方だけ書く（★公開の関所が食い違いで止まる★）★",
    ...
    {
        "why": "★壊れた合意も取り下げられる（★壊れた事実が健康な記録に見える★）★",
    ...
    {
        "why": "★取り下げた記録に理由が無くても、壊れた記録として扱わない★",
        "file": "scripts/repair_journal.py",
        "before": "        if len(str(rec.get(\"withdrawn_why\") or \"\").strip()) < MIN_WITHDRAW_WHY:",
        "after": "        if False:",
        "run": ["scripts/repair_journal.py"],
    },
    {
        "why": "★「押すかだけ見る」が日数の上限を見ない（★3日目以降も永久に「押さない」と答える★）★",
        "file": "scripts/mark_reviewed.py",
        "before": "    return len(_days_if_refused(st, slug, day)) > MAX_REFUSAL_DAYS, ng",
        "after": "    return False, ng",
        "run": ["scripts/mark_reviewed.py"],
    },
    ...
    # ─── 2026-09-26・当選契機の並びを天井と読んで公開した（台帳#716）───
    {
        "why": "★保証の言葉が別の『回目』に付いていても、スルー天井として採る（★当選契機の並びを天井として公開する★）★",
    ...
    # ─── 2026-09-25・既存の機種のカウンターの線も2AIの決定で書く ───
    {
        ...
        "file": "scripts/checker_verdict.py",
        "before": "            made = [n + u for n, u in _dn_num.numbers_with_unit(st)",
        "after": "            made = [n + u for n, u in []",
        "run": ["scripts/checker_verdict.py"],
    },
    {
    ...
    # ─── 2026-09-25・機種一覧の並べ替えだけのコミットは照合を求めない ───
    {
        ...
        "file": "scripts/pre_push_check.py",
        "before": "        return json.loads(text, object_pairs_hook=_sj._no_duplicate_keys,",
        "after": "        return json.loads(text,",
        "run": ["scripts/pre_push_check.py"],
    },
    {
        "why": "★並べ替えの判定を「重なりを数えない」比べ方にする（★1件を2つに増やして別の1件を消す形が通る★）★",
        "file": "scripts/pre_push_check.py",
        "before": "    return sorted(map(_key, a)) == sorted(map(_key, b))",
        "after": "    return set(map(_key, a)) == set(map(_key, b))",
        "run": ["scripts/pre_push_check.py"],
    },
    {
        "why": "★並べ替えの判定で、一覧の形かどうかを見ない（★辞書でもキーの並びを比べて通る★）★",
        "file": "scripts/pre_push_check.py",
        "before": "    if not (isinstance(a, list) and isinstance(b, list)):",
        "after": "    if False:",
        "run": ["scripts/pre_push_check.py"],
    },
    {
        "why": "★並べ替えの免除で、機種の記事が一緒に入っていても通す（★記事の書き換えが照合なしで出る★）★",
        "file": "scripts/pre_push_check.py",
        "before": "    if arts != [_ORDER_FILE] or not sha:",
        "after": "    if not sha:",
        "run": ["scripts/pre_push_check.py"],
    },
    {
        "why": "★並べ替えの免除で、合流コミットも見る（★片方の親とだけ比べて通る★）★",
        "file": "scripts/pre_push_check.py",
        "before": "    if len(par) != 2:",
        "after": "    if not par:",
        "run": ["scripts/pre_push_check.py"],
    ...
    {
        "why": "★照合の判断から、並べ替えだけの免除を外す（★夜の公開が0時をまたいだ朝は、並び順をまた押し出せない★）★",
        "file": "scripts/pre_push_check.py",
        "before": "    if order_only_commit(sha, paths, git=git):",
        "after": "    if False:",
        "run": ["scripts/pre_push_check.py"],
    },
    {
        "why": "★push前の照合が needs_check を通らず、古い判断に戻る（★判断の試験だけ緑のまま、本番の並べ替えが止まる★）★",
        "file": "scripts/pre_push_check.py",
        "before": "            if not needs_check(sha, _paths, lane):",
        "after": "            if not touches_articles(_paths):",
        "run": ["scripts/pre_push_check.py"],
    },
    {
        "why": "★触ったファイルを読むとき、名前の変更で元の置き場が消える読み方に戻す（★記事を見張りの外へ動かして並べ替えと混ぜると、照合なしで通る★）★",
        "file": "scripts/pre_push_check.py",
        "before": "    f = subprocess.run([\"git\", \"-C\", cwd or BASE, \"show\", \"--no-renames\",",
        "after": "    f = subprocess.run([\"git\", \"-C\", cwd or BASE, \"show\",",
        "run": ["scripts/pre_push_check.py"],
    },
    {
        "why": "★push前の照合が commit_paths を通らず、名前の変更を見落とす読み方に戻る★",
        "file": "scripts/pre_push_check.py",
        "before": "        _paths = commit_paths(sha)",
        "after": "        _paths = (_git(\"show\", \"--name-only\", \"--format=\", sha) or \"\").split()",
        "run": ["scripts/pre_push_check.py"],
    },
    ...
    # ─── 2026-09-15・落ち方を3分類にして2AIへ回す（台帳#675） ──────────
    {
        "why": "★DMM側の明白な食い違いを、硬い落ち方から外す"
               "（★知らない符丁は2AIへ回すので、機種名・メーカー・導入月が"
               "明白に違うページまで問いになる。しかもDMM側は控えに残せないので"
               "毎晩同じ問いが出続ける★＝Codexの重大2）★",
        "file": "scripts/model_code_lookup.py",
        "before": '    "DMM_PAGE_UNREADABLE",       # 本文の構造を読めない',
        "after": '    # "DMM_PAGE_UNREADABLE",',
        "run": ["scripts/model_code_lookup.py"],
        "issues": [675],
    },
    {
        "why": "★符丁の数え方を、素の \"...\" だけに戻す"
               "（★f\"...\" で作る符丁を数え落とし、分類の取りこぼしに"
               "気づけない★＝Codexの重大2）★",
        "file": "scripts/model_code_lookup.py",
    ...
    {
        "why": "★知らない落ち方を、黙って外す側（硬い）に倒す"
               "（★新しい落ち方が増えるたびに、機種が静かに消える★）★",
        "file": "scripts/model_code_lookup.py",
        "before": "    if any(c in HARD_REASON_CODES for c in cs):\n"
                  "        return \"HARD_REJECT\"\n"
                  "    return \"REVIEW\"",
        "after": "    if any(c in REVIEW_REASON_CODES for c in cs):\n"
                 "        return \"REVIEW\"\n"
                 "    return \"HARD_REJECT\"",
        "run": ["scripts/model_code_lookup.py"],
        "issues": [675],
    ...
    {
        "why": "★題の同定に落ちた時点で返す（メーカー欄の落ち方を符丁にしない）"
               "（★複合の落ち方が生成側から一度も出ない＝控えの器だけあって"
               "中身が無い★＝モンハンライズが止まっていた形）★",
        "file": "scripts/model_code_lookup.py",
        "before": "        if expected_maker:\n"
                  "            _mc_fail = maker_state(",
        "after": "        if False:\n"
                 "            _mc_fail = maker_state(",
        "run": ["scripts/model_code_lookup.py"],
        "issues": [675],
    ...
    {
        "why": "★同定に、取ってきた本文を渡さない（別々に取る）"
               "（★分類した本文と、材料にする本文が別物になる＝"
               "2AIが決めた控えが永久に効かない★）★",
        "file": "scripts/add_machine_run.py",
    ...
    #   「対象の根拠が何件でも通す」＝`_roles.count("target") != 1` を消す壊し方。
    #   ★観測できない★＝隣の守りが先に断る（罠④）。
    #     ・対象が2件 → 2件目のURLは target_url と違うので
    #       「対象と名乗る根拠が target_url と違います」で断る
    #     ・対象が0件 → 「対象ページが根拠に入っていません」で断る
    #   ★件数の検査は、その2つより分かりやすい理由を返すための厚み★。
    #   ★黙って消さずに理由を残す★＝緑にするために消したのではない。
    # ─── 2026-09-15・名鑑が足りなくても止めない（台帳#674） ──────────
    {
        "why": "★名鑑が1件しかないときの打ち切りを戻す"
               "（★読める1件も読まず、2AIへの問いも作られず、"
               "2AIが確定させた値の合流にも届かない★）★",
        "file": "scripts/add_machine_run.py",
        "before": ('                    "DMM単独の例外は使いませんでした: "'
                   ' + _other_why_here[:120])' + chr(10)
                   + '            # ★★ここで返さない★★'),
        "after": ('                    "DMM単独の例外は使いませんでした: "'
                  ' + _other_why_here[:120])' + chr(10)
                  + '            return got' + chr(10)
                  + '            # ★★ここで返さない★★'),
        "run": ["scripts/add_machine_run.py"],
    ...
    {
        "why": "★「名鑑が2件そろっていない」を、また公開を止める理由に戻す"
    ...
    {
        "why": "★取れないページで1件に減ったときの打ち切りを戻す"
               "（★2件見つかっても、途中で減れば同じ所で黙って終わる★）★",
        "file": "scripts/add_machine_run.py",
        "before": ('                "（取れない・転送されるページを除いた結果）")'
                   + chr(10) + "            # ★止めない★"),
        "after": ('                "（取れない・転送されるページを除いた結果）")'
                  + chr(10) + "            return got"
                  + chr(10) + "            # ★止めない★"),
        "run": ["scripts/add_machine_run.py"],
        "issues": [674],
    ...
    {
        "why": "★メーカー欄の照合で1件に減ったときの打ち切りを戻す"
               "（★モンハンライズが実際にここで止まっていた★）★",
        "file": "scripts/add_machine_run.py",
        "before": ('                "（2件以上が要る・メーカー欄の照合で除いた結果）")'
                   + chr(10) + "            # ★止めない★"),
        "after": ('                "（2件以上が要る・メーカー欄の照合で除いた結果）")'
                  + chr(10) + "            return got"
                  + chr(10) + "            # ★止めない★"),
        "run": ["scripts/add_machine_run.py"],
        "issues": [674],
    ...
    {
        "why": "★転載照合で取れずに1件へ減ったときの打ち切りを戻す★",
        "file": "scripts/add_machine_run.py",
        "before": ('                "（2件以上が要る・転載照合で取得できないページを除いた結果）")'
                   + chr(10) + "            # ★止めない★"),
        "after": ('                "（2件以上が要る・転載照合で取得できないページを除いた結果）")'
                  + chr(10) + "            return got"
                  + chr(10) + "            # ★止めない★"),
        "run": ["scripts/add_machine_run.py"],
        "issues": [674],
    ...
    {
        "why": "★控えを読めないことに符丁を付けない（付ける側を壊す）"
               "（★止める側は符丁で探すので、付けなければ素通りする★"
               "＝CodexのP1。止まる試験を `gather` ごと偽物にすると"
               "この壊し方は捕まらない）★",
        "file": "scripts/add_machine_run.py",
        "before": '            f"{MAKER_CACHE_UNREADABLE}メーカー照合の控えを読めません"',
        "after": '            f"メーカー照合の控えを読めません"',
        "run": ["scripts/add_machine_run.py"],
        "issues": [674],
    },
    {
        "why": "★メーカー照合の控えを読めないときの関門を外す"
    ...
    # ★★外した壊し方（2026-09-15・台帳#674）★★
    #   「材料の入れ物を『無い』に戻す」＝ _gather の初期値を None にする壊し方。
    #   ★観測できなくなった★＝同じ日に打ち切り4か所を全部やめたので、
    #   _gather は必ず読み取り器まで進んで material を入れ直す。
    #   ＝初期値を何にしても外から見える違いが無い。
    #   ★守り自体は残してある★（あとで早く返る道を足したときの受け皿）。
    #   ★いま効いている守りは呼ぶ側の normalize_material★で、
    #   そちらの壊し方（使う側での形の保証を外す）は登録してある。
    #   ★黙って消さずにここへ理由を残す★＝緑にするために消したのではない。
    {
        "why": "★使う側での形の保証を外す"
    ...
    },
    {
        "why": "★控えを甘い読み方で読む"
               ...
               "裏取り待ちの案件が閉じる）★",
        "file": "scripts/recheck.py",
        "before": "        got = _cv.load(require_exists=True)",
        "after": "        got = _cv.load(strict=False, require_exists=True)",
        "run": ["scripts/recheck.py"],
    },
    {
        "why": "★控えが読めないときに合格にする（確かめていないのに閉じる）★",
        "file": "scripts/recheck.py",
        "before": "        return _result(ERROR, f\"確定値の控えを読めません: {e}\", args)",
        "after": "        return _result(PASS, f\"確定値の控えを読めません: {e}\", args)",
        "run": ["scripts/recheck.py"],
    },
    {
        "why": "★見た中身の指紋を返さない（確かめてから閉じるまでに控えが差し替わっても気づけない）★",
        "file": "scripts/recheck.py",
        "before": "    _obs = {\"record_sha256\": _sha(json.dumps(rec, ensure_ascii=False,",
        "after": "    _obs = {\"record_sha256_\": _sha(json.dumps(rec, ensure_ascii=False,",
        "run": ["scripts/recheck.py"],
    },
    {
    ...
    # ★★「書く直前のセルの照合」は登録しない★★（2026-09-18）＝
    #   行や中身がずれる経路（同じ表を2回分ける／同じ表で分けてセルを直す／
    #   同じセルを2回直す）を**全部、計画の段階で断る**ようにしたので、
    #   あの照合はもう発火しない＝壊しても試験は赤くならない（罠㊻）。
    #   ★2026-09-18に一度「もう発火しない」と書いたが、そのときは誤りだった★＝
    #   同じセルを2回直す経路が残っていた（Codexの6回目で指摘・実際に到達した）。
    #   ★いまは断る側を全部登録してある★（下の3件）。照合は純粋な保険。
    {
    ...
    {
        "why": "★行を分ける操作と、同じ表のセル直しの同時実行を断らない"
               "（★ずれた先がたまたま同じ文字なら書く直前の照合も通り、"
               "挿入された別のセルを書き換える★）★",
        "file": "scripts/decide_now.py",
        "before": "            if _at:",
        "after": "            if False:",
    ...
    {
        "why": "★同じ表を2回分けるのを断らない"
               "（★1回目で行がずれ、ずれた先が同じ文字なら"
               "挿入された行のほうを分けてしまう＝本来の行は残る★）★",
        "file": "scripts/decide_now.py",
        "before": "        if len(_ris) > 1:",
        "after": "        if False:",
        "run": ["scripts/decide_now.py"],
        "issues": [698],
    },
    {
        "why": "★同じ場所を2回直すのを断らない"
               "（★計画はどれも変更前の記事から組まれるので全部通り、"
               "本文なら黙って後のものだけが効き、セルなら書く時だけ失敗する★）★",
        "file": "scripts/decide_now.py",
        "before": "        if len(_as) > 1:",
        "after": "        if False:",
        "run": ["scripts/decide_now.py"],
...
def check(only: str = "", fast: bool = False, only_index=None) -> int:
    ...
    try:
        ...
        tried = 0
        _pristine: dict = {}       # ★ファイルごとの「壊す前の中身」★
        _green: dict = {}          # ★試験の組ごとの「壊す前の結果」★
        for i, m in enumerate(MUTATIONS, 1):
            ...
            # ★★「壊す前の姿」は1回だけ覚える★★（2026-09-15）
            #   ★毎回の書き戻しが効いていることを、ここで実際に確かめる★
            #   （覚えた中身と違っていたら、前の壊し方が残っている）。
            ...
            # ★★同じ「壊す前」を何度も測らない★★（2026-09-15）
            #   ★なぜ速くする必要があったか★＝守りが増えて、
            #   手元でCIを再現する道具の制限時間（30分）の97%まで来ていた
            #   （実測1742秒）。少しでも他の処理が重なると時間切れになり、
            #   ★CIの結果を先に確かめる手段そのものが失われる★。
            #   ★制限時間は上げない★＝遅くなったことを隠すだけ。
            #   ★安全な理由★＝壊した中身は必ず元へ戻すので、
            #   「壊す前の姿」は全部の壊し方で同じ。
            #   ★覚えた姿と違っていたら、覚えを使わずに測り直す★
            #   （前の壊し方が残っている＝そのときは元の動きに戻る）。
            _key = tuple(m["run"])
            if src == _pristine[p] and _key in _green:
                _red, _why = _green[_key]
            else:
                _red, _why = _run_tests(root, m["run"])
                if src == _pristine[p]:
                    _green[_key] = (_red, _why)
            if _red:
                print(f"  ★ND {i}. {m['why']}"
