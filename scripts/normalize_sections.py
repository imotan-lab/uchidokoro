...
IDEAL_ORDER = [
    "天井・恩恵",
    "基本スペック",
    # ★★モード・ゾーン★★（2026-09-02・台帳#523の②・運営者の要望）
    #   ★打ち手が読む順★＝天井 → いまどこ → 何Gから → どこでやめる。
    #   2番目（いまどの状態か）が丸ごと無かったので、
    #   「650Gから狙い目」の**なぜ**が読めなかった。
    #   ★必須の箱ではない★＝未確認なら欄ごと出さない（運営者の判断）。
    #   ここに在るのは「在るなら、この位置に並べる」という意味。
    "モード・ゾーン",
    "当サイトの狙い目",
    "朝一・リセット情報",
    "狙い目の根拠",
    "ヤメ時の判断",
    "立ち回りのコツ",
    # ★★新台の並びとそろえた★★（2026-09-18・運営者の指示）
    #   ＞ 項目は多いほうがいいね　そっちでまとめて
    #   ★直す前★＝新台は「ゲーム性」「確認できたCZ」を必ず作るのに、
    #   既存記事の並びにはこの2つが無かった＝
    #   ★新台が育っても既存記事の形にならなかった★。
