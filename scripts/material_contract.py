...
CONTRACT_INPUTS = frozenset({
    # ★★試験の間だけ記録を止める決まり★★（2026-09-13・台帳#655）
    #   ★なぜ材料の契約に入れるのか★＝採否そのものには触らない
    #   （呼ぶのは `selftest()` の中だけ）。ただし壊れると
    #   ★本番でログが1行も残らなくなる★形の事故が起こり得る
    #   （既定を「試験中」にする・終わったあと戻さない）。
    #   記録が消えると、新台タスクが止まった晩に原因を追えない。
    #   ＝採否を**追跡できるかどうか**に効くので、変えたらレビューに載せる。
    "scripts/selftest_log_guard.py",
    ...
    # ★★2AIが実際に読む本文そのものを決める側★★（2026-09-02）
    #   どのページを集めるか／どれを「読めた」とみなすか／
    #   ★2AIに渡す本文★ を決めている。
    #   ここが書き換われば、2AIの判断の前提が丸ごと変わる。
    "scripts/mode_ask.py",
    # ★★控えた判断が読者に届くかの最後の関門★★（2026-09-02）
    #   いまの証拠を確かめ直すか／控えを使ってよいか／
    #   ★記事に節を入れるか・差し替えるか・消すか★を決める。
    #   野放しだと、承認をやり直さずに「何が公開されるか」を変えられる。
    "scripts/mode_apply.py",
    # ★★本文を取り出す側★★（2026-09-02・監査50が検知）
    #   mode_ask はこれで★2AIが実際に読む本文★を作る。
    #   ここが変われば、読む文章・証拠の指紋・引用の照合が全部変わる。
    #   ★関門だけを守っても、その関門が信用している側が野放しなら意味がない★
    ...
    # ★★機械が読めなかったときに2AIへ回す道★★（2026-09-10）
    #   read_failure … 失敗を「型のついた出来事」にする
    #     ＝ここが空を返すと、失敗が問いにならず静かに止まる
    #   page_reading … 2AIの答えを控え、逐語がそのページに在るか照合する
    #     ＝ここが甘くなると「材料に使ってよいページ」の判断が緩む
    "scripts/read_failure.py",
    "scripts/page_reading.py",
