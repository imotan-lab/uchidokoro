...
_COND_PATTERNS = (
    ...
    r"(非当選|非経由|未経由)",
    r"[0-9]周目",
    # ★実データ較正で判明した条件表現（2026-07-21）★
    #   lenient 側でも確実に落とすため、条件語として正式に登録する。
    r"設定[0-9lv]のみ|設定[0-9lv]限定|偶数設定|奇数設定|高設定|低設定|設定[0-9lv]以上",
    r"失敗時|成功時|非成功|不成功",
    r"裏天井|仮天井|第[0-9]天井|天井[a-e](?![0-9a-z数ゲ])|サブ天井|疑似天井",
    r"モード天井|ゾーン天井|区間天井",
    r"[0-9]回目のみ|[0-9]回目限定",
)
_COND_RE = None
...
def _value_pat(ceiling_type: str) -> str:
    """同単位の「値」を数えるための共通パターン（★桁区切り 1,000 も1つの値として数える★
    2026-07-21 Codex15巡目 指摘1）。"""
    return rf"(?<![0-9.,])[0-9][0-9,]*\s*(?:[＋+]\s*[αa])?(?:{_unit_pat(ceiling_type)})"
...
def quote_splits(quote: str, value, ceiling_type: str, level: str = "strict"):
    ...
    # ★lenient のときだけ、スルー天井の言い回しを広げる★
    #   「4スルー後5回目の当選」「最大7回のBCスルー後」等、実際の解析記事で頻出。
    #   同じ文に「スルー」または「回目」がある場合に限り「回/連続」も単位と認める。
    if level == "lenient" and (ceiling_type or "") == "through" and             re.search(r"スルー|回目", n):
        up = f"(?:{up}|回|連続)(?![転数分列個])"
    out = []
    for v in vals:
...
def main_ceiling_quote(quote: str, value, cores=(), ceiling_type: str = "game",
                       level: str = "strict") -> str | None:
    """主天井の記述だと肯定的に確認できなければ理由を返す（＝修正に使わない）。

    ★2026-07-21 Codexとの折衷設計（証拠契約）★
      level="strict" : 従来どおり。値の前が定型語だけで説明できること（残余0文字）まで要求
      level="lenient": 「同じ意味の主張をしているか」だけを見る。すなわち
                       ・単一の文であること
                       ・値が単位付きで存在すること
                       ・天井を表す語が【その値に係って】いること
                       ・値の前後に限定条件・注記・非断定表現が無いこと
                       を要求し、値の前の自由記述（「内部的な滞在モードに応じて」等）は許す。
      自動修正には「2ドメインが同じ主張」かつ「少なくとも1件が strict」を要求する。
      片方を lenient にすることで、書き方の違いによる取りこぼしだけを救う。
    """
    ...
    splits = quote_splits(quote, value, ceiling_type, level)
    if not splits:
    ...
    for prefix, suffix in splits:
        ...
        # ★天井語が【この値】に係っているかを確認する（Codex15巡目 指摘1）★
        #   値より後ろにある天井語は、その後ろに別の同単位の値があれば
        #   そちらに係っている（「狙い目は800G 通常時の天井は1,000G」）。
        def _attached(rx):
            if rx.search(clause_pre):
                # 値より前の天井語：その間に別の値が無いこと
                last = max(m.end() for m in rx.finditer(clause_pre))
                return not re.search(_value_pat(ceiling_type), clause_pre[last:])
            m = rx.search(clause_suf)
            if not m:
                return False
            return not re.search(_value_pat(ceiling_type), clause_suf[m.end():])

        strong = _attached(_CEILING_RE)
        # ★スルー天井の標準表現（lenientのみ）★
        #   「4スルー後5回目の当選で…」「最大7回のBCスルー後、8回目の…」＝
        #   スルー天井はこの書き方が普通で、「天井」の語が出ないことが多い。
        through_form = (level == "lenient" and (ceiling_type or "") == "through"
                        and bool(re.search(r"[0-9]+回目|スルー後|スルー到達", clause)))
        weak = through_form or _attached(_CEILING_WEAK_RE) and (
            "通常時" in clause
            # ★lenient のみ★「最大○○G消化で当選」型（通常時の明示が無い書き方）を許す。
            #   別挙動の語（突入・移行・終了等）がある節は下の検査で落ちる。
            or (level == "lenient" and re.search(r"最大|max", clause)))
        ...
        if not strong and not through_form:
        ...
        if m_note:
            ...
            # ★注記の記号は ※ ／ / ： :（読点は含めない）★
            #   読点の後ろは多くの場合「恩恵の説明」（…に到達し、STに当選する）であり
            #   注記ではない。ただし読点の後ろでも「のみ・だけ・限定」等の限定表現が
            #   あれば条件の断り書きとみなす（「天井は800G、設定6のみ」）。
            rest_sentence = re.split(r"[。！？]", suffix or "")[0]
            m_note2 = re.search(r"[※／/：:]\s*([^。！？]{1,40})", rest_sentence)
            if not m_note2:
                m_note2 = re.search(r"、\s*([^。！？、]{1,40}(?:のみ|だけ|限定|に限る))",
                                    rest_sentence)
            if m_note2:
...
def classify(site_claims: list[dict], codex_claims: list[dict],
    ...
    for r in comparison:
        ...
        # ★証拠契約（2026-07-21 Codexとの折衷・運営者承認）★
        #   ・全ての出典が「同じ意味の主張」をしている（lenient を全件通る）
        #   ・そのうち★少なくとも1件★は厳格な検査（strict）を通る
        elif any(main_ceiling_quote(e.get("raw_quote"), codex.get("value"), cores,
                                    site.get("ceiling_type") or "", "lenient")
                 for e in ev_used):
            bad = next(x for x in (main_ceiling_quote(
                e.get("raw_quote"), codex.get("value"), cores,
                site.get("ceiling_type") or "", "lenient") for e in ev_used) if x)
            why = f"引用を主天井の記述と確認できない（{bad}）"
        elif not any(main_ceiling_quote(e.get("raw_quote"), codex.get("value"), cores,
                                        site.get("ceiling_type") or "", "strict") is None
                     for e in ev_used):
            why = "厳格な検査を通る出典が1件も無い（全て書き方が曖昧）"
...
def selftest() -> int:
    ...
    # 9.989 証拠契約（1件は厳格・全件は意味検査）— Codexとの折衷・運営者承認
    strict_q = "通常時を最大{v}G+α消化で天井到達"
    lenient_q = "内部的な滞在モードに応じて天井G数を管理しており、最大{v}G+αで当選する"
    cond_q = "設定変更時は天井G数が短縮され、{v}G+αで天井到達となる"

    def _pair(q1, q2, value=1000):
        c = C(K, value)
        c["evidence"] = [{"source_url": "https://a.com/x", "raw_quote": q1.format(v=value)},
                         {"source_url": "https://b.com/x", "raw_quote": q2.format(v=value)}]
        c["evidence_results"] = [{"source_url": "https://a.com/x", "verified": True},
                                 {"source_url": "https://b.com/x", "verified": True}]
        return classify([S(K, 900)], [c], [R(K, "UNKNOWN", "numeric_divergence")])

    eq(len(_pair(strict_q, lenient_q)["fix_candidates"]), 1,
       "1件が厳格・もう1件が意味検査なら修正候補になる")
    eq(len(_pair(strict_q, strict_q)["fix_candidates"]), 1, "2件とも厳格でも当然可")
    eq(len(_pair(lenient_q, lenient_q)["fix_candidates"]), 0,
       "厳格を通る出典が1件も無ければ修正しない")
    eq(len(_pair(strict_q, cond_q)["fix_candidates"]), 0,
       "片方が条件つき天井なら修正しない")
