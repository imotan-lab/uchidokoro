"""new_machine_watch.py — 出典のページを、決まりを守って取ってくる係。

★★2026-08-21に役目が変わった★★（台帳#379の【4】・Codexの指摘）
  もとは「メーカー公式の機種一覧を見て、新台を見つける」係だった。
  ★その仕組みは2026-08-16に丸ごと消した★（台帳#377）＝
  出典は大手サイトへ寄せると決めたため。
  ★新台を見つけるのは dmm_discover.py（DMMぱちタウンの導入カレンダー）★

  2026-08-21に、残っていた `scan_maker()` と CLI（--scan / --check）も消した。
  ★止めたまま残さない★＝残すと「まだ生きている」と誤読される
  （実際、この説明文が残っていたせいでCodexから指摘を受けた）。

★いまこのファイルが持っているもの★
  ・guarded_open / _get … ★通信の関所★（名簿にある先へ、名乗った用途でだけ行く）
  ・fetching(用途) … 何のために取りに行くのかを名乗る囲み
  ・redirect_problem … 転送先が別のサイトになっていないか
  ・product_urls / machine_name / release_month / is_recent … ページから読む部品
  ・page_is_machine ほか … 同定の部品

★人が保守するのは assets/data/maker-catalogs.json★
  ただし役目は「メーカー名から社を同定する」ことだけ。
  ★巡回先のURL（list_url）は持たない★（0/25件）。

★黙って0件にしない★
  取れなくなったときに「新台なし」と誤認して静かに止まるのが一番こわい。
  取れなかったことは、取れなかったこととして返す。
...
def main() -> int:
    # ★メーカー公式を巡回する道は無い★（2026-08-21・台帳#379の【4】）
    #   2026-08-16に仕組みごと消したのに（台帳#377）、
    #   --scan / --check と scan_maker だけが残っていた。
    #   現行の名簿に list_url が無いので実データでは到達しないが、
    #   ★設定を渡せば到達できてしまう★＝
    #   「止めたはずのものが、まだ動く形で残っている」状態だった。
    #   CLAUDE.md の「★止めたまま残さない★（残すと『まだ生きている』と
    #   誤読され、実際に誤報した）」に従って消す。
    #   ★新台の入口は DMMぱちタウン（dmm_discover.py）だけ★
