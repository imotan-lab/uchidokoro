...
#   ★★保証の言葉は「その回目」に付いていること★★（2026-09-26・台帳#716）
#     ★実データで誤読した★＝DMMの
#     「当選契機 1Gレンチャンス成功4回目・8回目など ※8回目は当選濃厚」から
#     「4回目」＋（14字以内の）「濃厚」で一致し、★3スルーの天井として記事に出た★。
#     保証（濃厚）が付いているのは**8回目**で、4回目ではない。
#     しかも「など」が示すとおり、これは天井ではなく**当選契機の並び**。
#     → 拾った「N回目」と保証の言葉のあいだに★別の「M回目」があれば採らない★
#       （どちらに付く保証か機械には決められない＝2AIの領分）。
_SENT_THROUGH_NTH = re.compile(
    r"(?P<counted>[^。、]{0,16}?)の?(?P<nth>\d{1,2})\s*回目"
    r"(?![^。]{0,6}?スルー)"
    r"(?:は天井|(?:(?!\d\s*回目)[^。]){0,14}?(?:必ず|確定|濃厚))")
...
def selftest() -> int:
    ...
    # ★★保証の言葉が別の「回目」に付いていたら採らない★★（2026-09-26・台帳#716）
    #   ★実データで誤読して記事に出した★＝DMM「彼女、お借りします」の
    #   当選契機の並びを天井と読み、★3スルー★として公開してしまった。
    t("★★保証の言葉が別の『回目』に付いていたら採らない★★"
      "（★これで当選契機の並びを天井と読み、3スルーを公開した★）",
      not [g for g in from_sentences(
          "当選契機 1Gレンチャンス成功4回目・8回目など ※8回目は当選濃厚")
          if g["kind"] == "THROUGH"])
