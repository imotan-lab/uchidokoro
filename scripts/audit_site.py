...
#   41_自動で通信してよい先: ★機械の安全の話で、読者に届くものではない★
#     （2026-08-16・台帳#377／依頼216）。ここで配信を止めると、記事とは
#     無関係な理由でサイトが落ちる（2026-08-12に項目31で実際に起きた）。
#     ★いま赤い理由★＝ちょんぼりすた・なな徹の規約をまだ確かめていない。
#     この2つは記事の材料として毎晩読んでいた先で、P-WORLD・一撃と
#     同じく**規約を読まないまま巡回していた**。
#     ★実際の通信は new_machine_watch._get が止めている★ので、
#     赤いあいだ材料は取りに行けない（fail-closed）。
#     ★タスクを再開する条件★＝ここが0件になること。
