...
def _check_56_selftest() -> list[str]:
    ...
    if _want56:
        ...
        # ★★見えない字は「文字」と数えないか★★
        #   （2026-09-03。★Codexの問いを自分で確かめたら出た★＝
        #     ゼロ幅スペース・制御文字・方向制御は「文字あり」と
        #     数えられていたが、PythonでもJSでも読者には見えない）
        #   ★名簿ではなくUnicodeの分類で決める★（Cf・Cc・Zs）
        #   ★★読者が見るのはJS側の描画★★（2026-09-03・Codexの5回目の指摘1）
        #   ★直す前はここが誤りだった★＝`<br>` と `&nbsp;` を
        #   「見える字」として**要求して**おり、
        #   ★PythonとJSのずれを正解として固定していた★。
        #   Python は先にHTMLをエスケープするので「文字」に見えるが、
        #   JS はエスケープしないので**読者には何も出ない**。
        for _label, _ch in (("ゼロ幅スペース", "\u200b"),
                            ("制御文字", "\x01"),
                            ("方向制御", "\u200e"),
                            ("全角スペース", "\u3000"),
                            ("狭い空白", "\u2007"),
                            ...
                            ("改行タグ", "<br>"),
                            ("HTMLコメント", "<!--x-->")):
            if _ba56b.visible_text(_ch):
                ng.append(f"★56の対照実験が落ちています★: "
                          f"{_label}を「文字あり」と数えます")
        # ★対照＝見える字は落とさない★（消しすぎない）
        for _label, _t in (("普通の文", "天井1200G"),
                           ("太字の中身", "**天井**"),
                           ("数字だけ", "1200"),
                           ("記号だけ", "※"),
                           ("タグの中の文字", "<span>本文</span>")):
    ...
    for path in sorted(glob.glob(os.path.join(det, "*.json"))):
        ...
        if n:
            ng.append(
                ...
                "「5.6枚」へ言い換えてください")
    # ★★機種一覧（チェッカーの注記）も見る★★（2026-09-09・台帳#605）
    #   ★直す前は記事データしか見ていなかった★ので、
    #   ★読者が画面で操作するチェッカーの注記に5機種残っていた★。
    #   そこは「5.0枚のつもりで5.6枚の値を見る」形になり、
    #   期待値を取り違える。
    for m in machines or []:
        _slug = str((m or {}).get("slug") or "")
        _txt = json.dumps(m, ensure_ascii=False)
        _n2 = _txt.count("5.6枚等価")
        if _n2:
            ng.append(
                f"{_slug}: 機種一覧に「5.6枚等価」が {_n2} 箇所あります"
                "（ガイドの定義は 等価=5.0枚 / 5.6枚 は別物）。"
                "「5.6枚」へ言い換えてください")
    return ng
