...
#   ★飾りを外してから見る★（2026-09-08）＝下の `_undecorate` を通す。
_LABEL = re.compile(r"^\s*([^：:。、\n]{1,40})\s*[：:]")

# ★項目名の長さの上限★（括弧の補足は数えない）
_LABEL_MAX = 14
# ★括弧の補足は長さに数えない★（2026-09-08・実データで測って決めた）
#   ★Codexの指摘（原則として正しい）★＝一律に落とすと
#   「この機種（朝一を除く）は天井狙いが有効：積極的に狙う」のような
#   **本物の文**がラベル扱いになり、文体の違反を見逃す。
#   ★実データで測ると逆だった★＝末尾の括弧だけ落とす形にすると、
#   ★本物のラベル7件が違反になる★（実測。設定別機械割・超電磁砲CHANCE・
#   AT&ボーナス間（実ゲーム数）での狙い目（等価）など）。
#   一方、Codexの挙げた形は★実データに1件も無い★。
#   ★この検査は手がかりであって関所ではない★（監査16・55は公開を止めない）。
#     見逃し … 2AIが記事を通して読むので拾える（そもそも0件）
#     誤検知 … ★基準値が書き直せなくなる★＝直した記録が残らず、
#              後日そのまま復活しても報告すら出ない（台帳#586そのもの）
#   → 一律に落とす。★数が増えたら測り直す★。
_PAREN_RUN = re.compile(r"[（(][^）)]*[）)]")

# ★飾りの記号★（太字の ** と ★）
#   ★意味ではなく記号の話★＝名簿ではないので、知らない言い回しでも効く。
_DECOR = re.compile(r"\*\*|★+")


def undecorate(text) -> str:
    """★飾りの記号を外す★（2026-09-08・台帳#586）

    ★これが無いと何が起きるか★（実測）
      ・「**この機種に天井はありません。**」＝行ぜんたいが太字の文は、
        「。」で切ると閉じの `**` だけが1つの文になり、違反と数えられた
      ・「★★確認できたモード・ゾーンのみ掲載しています★★」は、
        文末が `★` なので「です・ます」に見えなかった
      この2件のせいで、48件直しても★基準値を書き直せなかった★。

    ★判定にだけ使う★＝報告する文はそのまま
      （変えると本物の違反の印まで変わり、照合が総入れ替えになる）。
    """
    return _DECOR.sub("", str(text or ""))
...
def is_label_line(line: str) -> bool:
    ...
    """
    t = undecorate(line).strip()
    if not t or "。" in t:
        return False
    m = _LABEL.match(t)
    if not m:
        return False
    # ★括弧の補足は長さに数えない★（2026-09-08・台帳#586）
    #   「規定ゲーム数（前兆発生ポイント）」は16字だが、項目名は「規定ゲーム数」。
    #   ★上限そのものは残す★＝長い文の途中に「：」があるものを
    #   ラベル扱いにしないため。
    head = _PAREN_RUN.sub("", m.group(1)).strip()
    return 1 <= len(head) <= _LABEL_MAX
...
def ok_ending(sentence: str) -> bool:
    ...
    """
    # ★飾りを外してから見る★（2026-09-08・台帳#586）
    t = undecorate(sentence).strip()
    for _ in range(2):
...
def _check_text(out: list, slug: str, where: str, line, li=0) -> None:
    ...
    for x in sentences(line):
        # ★飾りだけの断片は文ではない★（2026-09-08・台帳#586）
        #   行ぜんたいが太字の文で、閉じの `**` だけが残る形。
        if not undecorate(x).strip():
            continue
        if ok_ending(x):
            continue
...
def selftest() -> int:
    ...
    # ★★飾りの記号で誤って違反にしない★★（2026-09-08・台帳#586）
    #   ★これが直る前は、48件直しても基準値を書き直せなかった★＝
    #   検査側の誤りが2件あるだけで、記録そのものが丸ごと止まっていた。
    #   ★どれも記号の構造の話で、意味の判定ではない★（名簿を増やしていない）。
    def _ng(line):
        _o = []
        _check_text(_o, "x", "本文", line)
        return [r["sentence"] for r in _o]

    t("★★行ぜんたいが太字の文を、違反にしない★★"
      "（★閉じの `**` だけが1つの文として数えられていた★）",
      _ng("**この機種に天井はありません。**") == [])
    t("★★★で囲まれた文を、違反にしない★★"
      "（★文末が ★ なので「です・ます」に見えなかった★）",
      _ng("★★確認できたモード・ゾーンのみ掲載しています★★") == [])
    t("★★項目名が長いラベルの行を、違反にしない★★"
      "（★括弧の補足まで数えて上限14字を超えていた★）",
      _ng("**規定ゲーム数（前兆発生ポイント）**：88G・168G・268G") == [])
    # ★★守りを弱めていないこと★★＝飾りを外しても、本物の違反は違反のまま
    t("★★飾りを付ければ通る、にはなっていない★★"
      "（★体言止めは、太字でも★付きでも違反のまま★）",
      _ng("天井到達で確定") and _ng("**天井到達で確定**")
      and _ng("★天井到達で確定★"))
    t("★括弧が真ん中にある文は、いまは拾えない★"
      "（★Codexの指摘・実データに0件なので直していない。"
      "末尾だけにすると本物のラベル7件が違反になる。数が増えたら測り直す★）",
      is_label_line("この機種（朝一を除く）は天井狙いが有効：積極的に狙う"))
    t("　括弧が2つあるラベルも、いままでどおりラベル"
      "（★これを文にすると実データで7件が違反になる★）",
      is_label_line("**AT&ボーナス間（実ゲーム数）での狙い目（等価）: 780G〜**"))
    t("　括弧つきの項目名はラベル",
      is_label_line("**規定ゲーム数（前兆発生ポイント）**：88G・168G・268G"))
    t("　長い文の途中に「：」があってもラベルにしない"
      "（★上限そのものは残している★）",
      not is_label_line("規定ゲーム数と前兆発生ポイントの関係について："
                        "詳しくは以下のとおりとします"))
    t("　飾りを外すのは判定だけ（報告する文はそのまま）",
      _ng("**天井到達で確定**") == ["**天井到達で確定**"])
    t("★ラベルと値の行は文体を求めない★",
