...
# ★数え上げに入れない手がかり★＝行ごとに出るので件数が大きくなり、
#   `counts` の意味が変わってしまう（`counts` は「直せる候補の数」）。
...
def gather(slug: str) -> dict:
    ...
    # ★★打ち方をすすめる書き方★★（2026-09-08・運営者の承認）
    #   ＞ 消してもいいよ。統一されていればそれでいいのよ。触るなとは言ってない。
    #   ★手がかりであって判定ではない★＝消すか言い換えるかは2AIが決める。
    #   ★数値・狙い目のラインは残す★（消すのは言い方だけ）。
    for _sec in out["article"]["sections"]:
        for _line in _sec.get("body") or []:
            _adv = [w for w in ADVICE_MARKS if w in _line]
            if _adv:
                out["hints"].append(
                    {"kind": "打ち方をすすめる言い方",
                     "section": _sec["title"], "words": _adv[:5],
                     "line": _line[:100],
                     "note": "読者に打ち方をすすめる書き方は消す"
                             "（2026-09-08・運営者の判断）。"
                             "★数値・狙い目のラインは残し、言い方だけ直す★。"
                             "★ふつうの言葉として使っている場合は触らない★"
                             "（2AIが記事を読んで決める）。"})
...
# ★★数値の読み方は1か所★★（2026-09-25・Codexの指摘）
#   ★符号は数値の一部★＝「+500枚」と「-500枚」は別の値。
#   ★符号のあとの空白も符号として読む★（「- 500枚」「▲ 500枚」。直す前は
#   符号が落ちて「500枚」と読み、出どころの検査に入らなかった）。
#   ★数字の直後の「-」は範囲の区切り★（「100-200G」）なので符号にしない。
#   ★カウンターの一覧の文の検査（checker_verdict）もこれを使う★（2か所に書かない）。
NUM_PATTERN = (r"(?<![\d.])[-−▲△+＋][ \u3000\t]*\d+(?:\.\d+)?"
               r"|\d+(?:\.\d+)?")
_SIGNS = "-−▲△+＋"
...
def _num_spans(s: str) -> list:
    """★数値の読み方の本体★ → [(終わりの位置, 符号つきの数値)]（空白は詰める）

    ★空白をはさんだ範囲（「100 - 200G」）の「-」は符号にしない★
    （2026-09-25・Codexの指摘）＝符号の前を空白を飛ばして見て、数字なら範囲。
    """
    import re
    s = str(s or "")
    out = []
    for m in re.finditer(NUM_PATTERN, s):
        tok = m.group(0)
        if tok[0] in _SIGNS:
...
def signed_numbers(s: str) -> list:
    """★符号つきの数値（単位は付けない・空白は詰める）★"""
    return [tok for _end, tok in _num_spans(s)]
...
# ★打ち方をすすめる言い方の手がかり★（2026-09-08・運営者の承認）
#   ★止める判断には使わない★＝2AIが記事を読んで決めるための手がかり。
#   ★実データで数えて選んだ★（513件・109機種）。
#   ★「着席」は入れない★＝69件あるが大半が「着席時」＝ふつうの言葉で、
#   機械には区別できない（＝名簿で判定してはいけない型）。
ADVICE_MARKS = (
    "積極的", "しましょう", "してください", "おすすめ",
    "べきです", "したいところ", "即ヤメせず", "狙いたい", "避けたい")
...
def apply_decision(path: str, apply_it: bool = False, *,
    ...
    for a in dec["actions"]:
        if a["op"] == "replace":
            ...
            # ★★意味の判断は2AIがする。機械は止めない★★（2026-09-25・運営者の方針）
            #   ＞ 既定は2AI。機械は不都合が起きたときだけ足す
            #   ★外したもの★＝数値の入れ替えの判定／記事に無い言葉／
            #   数値の前後の言い回しの一字一句一致。どれも機械が意味を判定して
            #   ★2AIが合意した直しを止めていた★（実例＝モンキーターンVの合意が
            #   9日間書けず、その機種の直しが全部止まっていた）。
            #   ★残すもの★＝出どころの逐語がサイトに在る／足す数値がその逐語に在る
            #   （数字を作らない）／言い回しが変わるなら2AIの理由を記録する。
            ...
            if nb != na:
                ...
                if src not in published:
                    result["problems"].append(
                        ...
                        f"{src[:44]!r}")
                    return result
                # ★足す数値は、出どころの逐語の中に全部あること（数字を作らない）★
                #   ★数え上げで見る★（2026-08-28・Codexの9回目）＝集合だと
                #   「同じ値が1件増える」変更が見えない。
...
def _selftest() -> int:
    ...
    try:
        ...
        t("★★見出しつきの行でも、2AIが合意した数値の置き換えを書ける★★"
          "（★直す前は見出しが係り先に入り、どの出どころとも一致しなかった★）",
          not rs9["problems"] and bool(rs9.get("done")))
        if rs9["problems"]:
            print("   断られた理由:", [p[:120] for p in rs9["problems"][:2]])
        # ★★残した守り★★＝足す数値が出どころの逐語に無ければ断る（数字を作らない）
        rs9b = apply_decision(dec_s9(
            {"op": "replace", "before": "**狙い目**：通常時410G〜",
             "after": "**狙い目**：5.6枚交換なら通常430Gから狙い目です。",
             "why": "わざと：出どころに無い数値",
             "numbers_from": "5.6枚交換なら通常420Gから狙い目です。",
             "meaning_why": "2AIで判断したことにしています（わざと）"}))
        t("★★出どころの逐語に無い数値は、今までどおり書かせない★★（数字を作らない）",
          any("出どころに無い数値" in p for p in rs9b["problems"]))
        rs9c = apply_decision(dec_s9(
            {"op": "replace", "before": "**狙い目**：通常時410G〜",
             "after": "**狙い目**：5.6枚交換なら通常420Gから狙い目です。",
             "why": "わざと：架空の出どころ",
             "numbers_from": "5.6枚交換なら通常420Gから狙い目です（架空）。",
             "meaning_why": "2AIで判断したことにしています（わざと）"}))
        t("★★サイトに無い出どころの逐語は、今までどおり受け取らない★★（言うだけでは通さない）",
          any("見つかりません" in p for p in rs9c["problems"]))
        rs9d = apply_decision(dec_s9(
            {"op": "replace", "before": "**狙い目**：通常時410G〜",
             "after": "**狙い目**：5.6枚交換なら通常420Gから狙い目です。",
             "why": "理由の記録が無い",
             "numbers_from": "5.6枚交換なら通常420Gから狙い目です。"}))
        t("★★言い回しが変わるなら、2AIの理由が無いと書かない★★（判断を記録に残す）",
          any("meaning_why" in p for p in rs9d["problems"]))
        ...
        #   ★守っているのは「数字を作らない」★（2026-09-25）＝符号は数値の一部なので、
        #   -500枚 は新しい数値。出どころの逐語が無ければ書かせない。
        ...
        t("★★書き換えで符号だけ変えるのも止める★★"
          ...
          bool(rs4["problems"])
          and "出どころが要ります" in "".join(rs4["problems"]))
        # ★★符号のあとに空白があっても符号として読む★★（2026-09-25・Codexの指摘）
        t("★★「- 500枚」「▲　500枚」も符号つきの数値として読む★★"
          "（★直す前は符号が落ちて「500枚」になり、出どころの検査に入らなかった★）",
          _numbers("差枚- 500枚") == ["-500枚"]
          and _numbers("差枚▲　500枚") == ["▲500枚"])
        t("　数字の直後の「-」は範囲の区切りなので符号にしない",
          _numbers("100-200G") == ["100-", "200G"])
        t("★★空白をはさんだ範囲（100 - 200G）の「-」も符号にしない★★"
