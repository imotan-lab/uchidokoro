...
# ★★書き出しも「です・ます」にそろえる★★（2026-09-02・運営者の判断「Aで。」）
#   ★直す前は後半が体言止め★（「…2026年10月5日 導入。」）で、
#   ★機械が作った時点で文体の決まりから外れていた★。
#   そのため育成で導入日が月→日に細かくなるたび、
#   文体の見張り（監査55）が「新しい違反」と見て書き込みを取り消し、
#   ★その機種の育成が必ず失敗していた★（台帳#552）。
LEAD_TEMPLATE = "{name}の機種情報ページです。登場時期は{release}です。"
...
#
# ★★新台と既存記事で、同じ並びにした★★（2026-09-18・運営者の指示）
#   ＞ 項目は多いほうがいいね　そっちでまとめて
#   ★直す前は別のリストだった★＝
#     新台 … 7箱（ゲーム性・確認できたCZが有り、狙い目の根拠・ヤメ時・
#            立ち回りのコツ・モード/ゾーンが無い）
#     既存 … 10項目（逆の顔ぶれ）
#   ＝★新台が育っても、既存記事の形にならなかった★。
#   正本＝`_design/items_2ai_2026-09-18.md`
SECTION_ORDER = ("天井・恩恵", "基本スペック", "当サイトの狙い目",
                 "朝一・リセット情報", "狙い目の根拠", "ヤメ時の判断",
                 "立ち回りのコツ", "ゲーム性", "確認できたCZ",
                 "設定示唆まとめ")
...
def _fmt_release(ymd: str) -> str:
    """登場時期の一文（2026-08-12・運営者決定／2026-09-02に文体をそろえた）。

      日まで分かる   → 2026年10月5日
      月までしか無い → 2026年10月頃（曖昧なままでよい）

    ★「登場時期は…です」の組み立ては呼び出し側★（2026-09-03）＝
      ここで文にすると、基本スペックの表が
      「登場時期｜登場時期は…です」と★項目名が二重★になる（試験が捕まえた）。

    ★「導入予定」とは書かない★＝導入後に嘘になる語なので検査で止まる。
    ★★「です」で終える★★（2026-09-02・運営者の判断「Aで。」）＝
      ★直す前は体言止め★（「…10月5日 導入。」）だったので、
      機械が作った時点で文体の決まりから外れ、
      ★導入日が細かくなるたびに育成が取り消されていた★（台帳#552）。
    """
    t = str(ymd or "")
    ...
    if m:
        return f"{m.group(1)}年{int(m.group(2))}月頃"
    return t
...
UNRESOLVED_MARKS = (
    ...
    # ★下位ページを全部は並べられなかった★（2026-09-08・Codexの指摘5）
    #   ★これを伝えないと「全部読んだ前提」で判断させてしまう★。
    ("下位ページを全部は並べられませんでした",
     "★読む先が全部ではありません（下位ページを並べきれていません）★"),
    # ★出典そのものが取れなかった★（2026-09-08・Codexの指摘）
    #   ★これを伝えないと、残った出典だけで「全部読んだ」ことにされる★。
    ("読めなかった出典があります",
     "★読む先が全部ではありません（取れなかった出典があります）★"),
)


def unresolved_questions(problems, urls=None, limit: int = 12,
    ...
    out, seen = [], set()
    # ★★「読む先が全部ではない」は必ず先に入れる★★
    #   （2026-09-08・Codexの指摘2）★直す前は問題の末尾に足していた★ので、
    #   先に12件たまると**この知らせだけが落ちた**＝
    #   いちばん材料が足りない機種で「全部読んだ前提」にさせていた。
    #   ★出典そのものを読めなかったほうも同じ★（2026-09-08・Codexの指摘）
    _INCOMPLETE = ("下位ページを全部は並べられませんでした",
                   "読めなかった出典があります")

    def _inc(p):
        return any(m in str(p or "") for m in _INCOMPLETE)

    _first = [p for p in (problems or []) if _inc(p)]
    _rest = [p for p in (problems or []) if not _inc(p)]
    for p in (_first + _rest):
        line = str(p or "").strip()
        if not line:
            continue
    ...
    if not out:
        return []
    # ★★頼み方はシンプルに★★（2026-09-08・運営者の指示）
    #   ＞ 指示しすぎて逆に狭めてるんだよ。ざっくりでいいんだよ。
    #   ＞ この台のこれ探せ サイトはこれとこれ。こんだけでいいんだよ。
    #   ★同じAIなのだから、人が人に頼むのと同じ書き方でよい★。
    #   細かく書くほど、そこに書いてある範囲しか見なくなる。
    #   ★渡すのは「何を」と「どこを」だけ★。
    #   読む先はその機種のページ全部（呼ぶ側が本体＋下位を入れて渡す）。
    where = [str(u) for u in (urls or []) if str(u or "").strip()]
    # ★★1つの問いにまとめる★★（2026-09-08・Codexの指摘3）
    #   ★直す前は [読む先だけの見出し, 問題1, 問題2, …] を返していた★。
    #   新台側は1件ずつ別の案件として台帳へ登録し、台帳は既定で1件だけ出す。
    #   ＝★最初の案件は「読む先」だけで何を探すか書いておらず、
    #     後の案件には読む先が無い★。分かれたら意味がない。
    lines = ["以下が読み取れませんでした。出典を読んで決めてください。"]
    lines += ["  ・" + x for x in out]
    if where:
        # ★★切らない★★（2026-09-08・Codexの指摘2）
        #   ★直す前は40本で切っていた★＝なな徹は1機種で43本あるので
        #   落ちる。狙い目のページが41本目以降なら、直した意味が無くなる。
        # ★★「全部」と書くのは、本当に全部のときだけ★★
        #   （2026-09-08・Codexの指摘2の2回目）★直す前は無条件だった★ので、
        #   同じ問いの中に「★読む先が全部ではありません★」と
        #   「読む先（全部）」が同居していた。
        lines.append(("読む先（全部）: " if complete else
                      "読む先（★全部ではありません★"
                      "下位ページを含めて、その機種のページを"
                      "一通り見てください）: ") + " ".join(where))
    # ★このサイト固有の決まり★（2026-09-08・Codexの指摘6で書き戻した）
    #   ★短くするときに落としていた★＝材料が欠けていても
    #   「読んで決めてください」とだけ言うと、不完全なまま断定させる。
    lines.append("★確認できなければ決めません★"
                 "（独立した発行元2つで一致しない値は載せません）")
    return ["\n".join(lines)]
...
def selftest() -> int:
    ...
    t("★★読めなかったものが質問になる★★"
      ...
      len(_uq) == 1 and "読む先" in _uq[0]
      and "取り出せませんでした" in _uq[0]
      and "1つの出典にしか" in _uq[0])
    # ★★「何を」と「どこを」は同じ問いに入れる★★（2026-09-08・Codexの指摘3）
    #   ★分けると台帳で別の案件になり、既定で1件しか出ないので、
    #     片方には読む先が無く、もう片方には何を探すかが無い★。
    t("★★「何を」と「どこを」が同じ問いに入っている★★"
      "（★分けると台帳で別々の案件になり、どちらも役に立たない★）",
      len(_uq) == 1
      and _uq[0].index("取り出せませんでした") < _uq[0].index("読む先"))
    t("　★読む先を途中で切らない★"
      "（★なな徹は1機種で43本ある＝切ると狙い目のページが落ちる★）",
      all(("https://example.invalid/%d" % i) in
          unresolved_questions(_pr, ["https://example.invalid/%d" % j
                                     for j in range(60)])[0]
          for i in (0, 41, 59)))
    # ★★問題が多い機種でも「全部ではない」が落ちない★★
    #   （2026-09-08・Codexの指摘2）★直す前は問題の末尾に足していた★ので、
    #   先に12件たまると**この知らせだけが落ちた**＝
    #   いちばん材料が足りない機種で「全部読んだ前提」にさせていた。
    _many = ["天井%d: 記述はあるが採れませんでした" % i for i in range(20)]
    _many.append("下位ページを全部は並べられませんでした（試験）")
    t("★★読み取り失敗が20件あっても、"
      "『読む先が全部ではない』は必ず出る★★"
      "（★落ちると、いちばん材料が足りない機種で全部読んだ前提にさせる★）",
      "読む先が全部ではありません" in unresolved_questions(_many, ["u"])[0])
    # ★出典そのものを読めなかった側も、同じように落とさない★
    #   （2026-09-08・Codexの指摘）
    _many2 = ["天井%d: 記述はあるが採れませんでした" % i for i in range(20)]
    _many2.append("読めなかった出典があります（取れない）: example.com")
    t("★★出典を読めなかったことも、問題が多い機種で落とさない★★",
      "取れなかった出典があります" in unresolved_questions(_many2, ["u"])[0])
    t("★★確認できなければ決めない、を必ず添える★★"
      "（★材料が欠けていても『読んで決めてください』だけだと断定させる★）",
      "確認できなければ決めません" in _uq[0]
      and "独立した発行元2つ" in _uq[0])
    ...
    t("　★同じ理由を二重に出さない★",
      unresolved_questions([_pr[0], _pr[0]])[0].count("・★") == 1)
    ...
    # ★★2AIの確定値からも書く★★（2026-09-09・台帳#601）
    #   ★直す前は書かなかった★ので、2AIで正しく確定させた瞬間に
    #   「機械割◯〜◯%」の行が消え、見張りが書き込みを止めた
    #   ＝★確定させるほど、その機種が公開できなくなる★（実測6機種）。
    #   ★いまの線は「裏付けの強さ」★＝どこから来た値かは問わない。
    t("★★2AIの確定値からも機械割の範囲を書く★★"
      "（★書かないと、正しく確定させた機種ほど公開できなくなる★）",
      payout_range_view({"payout_rate": {**IM, "_from": "confirmed_values",
                                         "value": {"1": "97.3%",
                                                   "6": "110.5%"}}})
      [:2] == (97.3, 110.5))
    t("　（対照）数えられる裏付けが無ければ書かない"
      "（★判定書が「未確認」と言うのに記事が書くと食い違う★）",
      payout_range_view({"payout_rate": {"_from": "confirmed_values",
                                         "value": {"1": "97.3%",
                                                   "6": "110.5%"}}}) is None)
