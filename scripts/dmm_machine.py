...
#   ★2026-08-23に読み方を変えた★＝以前は「（メーカー公式サイト）」で
#   切っていたが、**その案内が無いページがある**（SANKYO）。
#   いまは「社名が先頭と案内の直前の2か所に出る」ことを使って突き合わせる。
_MAKER_CUT = re.compile(r"[（(]\s*メーカー公式サイト\s*[）)]")
#   ★取り切れたか確かめる語★（残っていたら社名ではない＝止める）
_MAKER_LEFTOVER = ("掲載機種一覧", "公式サイト", "一覧")
# ★社名の直後に必ず付く案内★（この直前と先頭の2か所で社名を突き合わせる）
_MAKER_TAIL = "の掲載機種一覧"
...
def _maker_of(raw: str) -> str:
    ...
    ★★「切る」のをやめて「説明しきれるか」で決める★★（2026-08-23）
      ★直す前★＝「（メーカー公式サイト）」という**案内の印で切って**いた。
      ところがその案内が**無いページがある**（実測）:

        SANKYO     'SANKYO SANKYOの掲載機種一覧'                     → ★1文字も切れず落ちる★
        オーイズミ  'オーイズミ(メーカー公式サイト) オーイズミの掲載機種一覧' → 通る
        コナミ      'コナミアミューズメント(メーカー公式サイト) …'          → 通る

      9機種で測って1件（Lパチスロ からくりサーカス2＝SANKYO）が
      ★同定できず、その機種は永久に作れない★状態だった。

      ★印を1つ足すのは2段目の場合分け★（うちの鉄則で禁じている形）なので、
      **候補の社名で、残り全部を説明できるか**を見る形にした。

        頭から順に候補を伸ばし、残りが
          ［（メーカー公式サイト）］＋［{候補}の掲載機種一覧］
        だけで**きれいに無くなる**なら、その候補が社名。

      ＝飾りが有っても無くても、片方だけでも同じ規則で読める。
      ★「説明できたか」を見るので、切り取りより厳しい★
      （残りが1文字でも余れば採らない）。

      ★★危うく別のものを壊すところだった★★
      はじめ「社名は先頭と『の掲載機種一覧』の直前の2か所に出る」で書いたが、
      それだと ★一覧リンクが無い形（'平和（メーカー公式サイト）'）が落ちた★。
      既存の試験がそれを捕まえた＝SANKYOを直して平和を壊す寸前だった。
    """
    ...
    for k in range(1, len(s) + 1):
        ...
        if not cand:
            continue
        rest = s[k:].strip()
        # ★飾りは「その社名で作られたもの」だけ剥がす★
        rest = _MAKER_CUT.sub("", rest, count=1).strip()
        if rest.startswith(cand + _MAKER_TAIL):
            rest = rest[len(cand + _MAKER_TAIL):].strip()
        if not rest:                      # ★全部説明できた＝それが社名★
            head = cand
            break
    if not head:
        raise MachineError(
            f"メーカー欄から社名を取れません: {s[:50]}"
            "／★残りを社名の飾りとして説明できません★")
...
def selftest() -> int:
    ...
    # ★★公式サイトの案内が無いページ★★（2026-08-23・実測で見つけた）
    #   ★これが無いと、その社の機種は永久に作れない★
    #   実データ: Lパチスロ からくりサーカス2（DMM 5019・SANKYO）
    #   9機種を測って1件がこの形だった。
    def _maker_is(raw, want):
        """★例外で落ちても ❌ として数える★（後ろの試験を止めないため）"""
        try:
            return _maker_of(raw) == want
        except Exception:                                    # noqa: BLE001
            return False

    t("★★公式サイトの案内が無くても社名を取れる（SANKYO）★★"
      "／これが落ちると、その社の新台は永久に作れない",
      _maker_is("SANKYO SANKYOの掲載機種一覧", "SANKYO"))
    t("　社名に空白が入っていても読める",
      _maker_is("AB CD(メーカー公式サイト) AB CDの掲載機種一覧", "AB CD"))
    t("　案内が何も付かない形も読める",
      _maker_is("ユニバーサルブロス", "ユニバーサルブロス"))
    t("★★別の社の一覧が混ざっていたら採らない★★"
      "／候補で残りを説明しきれない形は止める",
      raises(lambda: _maker_of("SANKYO 平和の掲載機種一覧"), "形が変わった")
      or raises(lambda: _maker_of("SANKYO 平和の掲載機種一覧"), "説明できません"))
