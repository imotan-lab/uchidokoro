...
def directory_page_ok(name: str, html: str):
    ...
    ★2段で見る★（実データで測ってから決めた）
      ①題で同定できる → 通す（従来どおり厳しい）
      ②題で無理でも、**本文に正式名称の芯がある** → 通す
      ③どちらも無理 → 通さない（候補として人と2AIへ回す）

    ★なぜ②が要るか★＝大手には題に**通称しか入れない**ところがある
      （なな徹「【カバネリ2 海門決戦(スマスロ)】…」・DMMも一部）。
      実データ8機種で測ると、①だけでは29ページ中6ページ（約2割）が落ち、
      その6件は**全部が正しいページ**だった。②で6件とも救えた。
      ★これは場合分けではなく、誰にでも同じ1つの規則★
      （発行者ごとの例外や通称の辞書は作らない）。
    ...
    if not core:
        return False, "機種名の芯を取れません"
    body = _ci.normalize_core(" ".join(_w._visible_text(html or "").split()))
    if core in body:
        return True, "題は通称だが本文に正式名称がある"
    return False, why
...
def selftest() -> int:
    ...
    try:
        ...
        t("★★題が通称でも、本文に正式名称があれば通す★★（台帳#309）"
          "＝実データ8機種で測ると、題だけでは29ページ中6ページが落ち、"
          "その6件は全部が正しいページだった",
          directory_page_ok(name, nick) == (True, "題は通称だが本文に正式名称がある"))
