...
def assess(slug: str, repairing: bool = False) -> dict:
    ...
    #
    # ★★この段階は「止まっている」のではない★★（2026-08-21・台帳#440）
    #   IDENTITY_PENDING は task_guard.WRITABLE_STAGES に入っているので、
    #   ★更新タスクは触れる★（記事を直せる）。READY に届かないだけ。
    #   ★公開済みの記事に READY は要らない★（もう公開されている）。
    #   実測（2026-08-21・全130機種）: identity が揃っているのは10機種だけで、
    #   残り120機種は全部ここに入る。それでも毎日1〜3機種は直せている。
    #
    # ★★次にやることの文面を実行できるものへ直した★★
    #   直す前は「メーカー公式の商品ページと公安委員会の型式公示から取る」
    #   と書いていたが、★どちらもいまは使えない★＝
    #     ・メーカー公式の巡回は2026-08-16に仕組みごと削除（台帳#377）
    #     ・日工組は免責事項で自動取得できない（人が読むだけ）
    #   ＝★書いてあるとおりにやる方法が無い指示★だった。
    #
    # ★いま本当にできること★
    #   DMMの機種ページには型式名が載る（未導入の新台には無い）。
    #   ただし slug と機種ページURLの対応は slug_binding.py に
    #   **移行済み7件だけがコードに固定**されていて増やせない。
    #   ＝残りの機種は、まず2AIで機種ページを同定して
    #     machine_sources へ記録するところから。
    missing = ci.identity_missing(machine)
    if missing:
        out["stage"] = "IDENTITY_PENDING"
        out["reasons"] = [f"型式の同定情報が足りません: {missing}"]
        out["next_action"] = (
            "★この段階でも記事は直せる（READYに届かないだけ）★。"
            "identity を埋めるなら、DMMの機種ページで型式名を確かめてから "
            "machine_sources へ記録する（2AIで同定する）。"
            "★メーカー公式・日工組からは取れない★"
            "（巡回は2026-08-16に削除／日工組は人が読むだけ）")
        return out
