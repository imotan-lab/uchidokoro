...
def selftest() -> int:
    ...
    t("★★G が付いていない数値は触らない★★（周期・スルー回数）",
      ...
      == "CZ間300G〜 / 5周期目〜 / 2スルー〜")
    # ★★狙った守りだけが効く材料にする★★（2026-09-11・罠④）
    #   ★直す前は「等価250G〜」で試していた★＝この書き方には
    #   モードの呼び名が入っていないので、★手前の守り
    #   （「どのモードの話か分かりません」）が先に断って★いた。
    #   ＝この守りを消しても試験は緑のままだった（実際にそうなった）。
    #   ★モードの呼び名を入れて、手前の守りを通り抜けさせる★。
    _r1 = plan({"slug": "x", "checker": ck, "strategy": "等価CZ間250G〜"})
    _r2 = plan({"slug": "x", "checker": ck, "strategy": "5.6枚CZ間250G〜"})
    t("★★一覧が交換率を名乗っていたら触らない★★"
      "（呼び名と中身が食い違うため・実測8機種）"
      f"（理由: {_r1['why'][:24]}）",
      _r1["moves"] == [] and _r2["moves"] == []
      # ★断った理由まで見る★（罠㉚）＝別の守りが断っていたら赤くする
      and "交換率ごとに書き分け" in _r1["why"]
      and "交換率ごとに書き分け" in _r2["why"])
    # ★（対照）呼び名が無ければ、この材料は実際に書き換わる★
    #   ＝上の試験が「たまたま何も起きない材料」でないことを示す
    t("　（対照）呼び名を外すと書き換わる",
      plan({"slug": "x", "checker": ck,
