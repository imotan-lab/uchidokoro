...
def grant_for(decision, pages) -> dict:
    ...
    直す前は試験が許可証を**手作り**していたので、
    ここを空に壊しても試験が通った（接続部分を試していない＝罠）。
...
def _maker_related(expected: str, seen: str) -> bool:
    """★名鑑のメーカー欄が「関係のある社」か★（2026-09-12・Codexの指摘）

    問いの文を許可条件とそろえるために使う（採否はここで決めない）。
    """
    try:
        import model_code_lookup as _mcl2
        _ow = _mcl2._maker_core_owners(_mic.key_of(seen))
        return bool(_ow) and str(expected) not in _ow \
            and _mcl2._related(str(expected), _ow)
    except Exception:                      # noqa: BLE001
        return False
...
def _review_question(r, url, maker, machine_name, release_date,
    ...
    _obs = [
        ...
        f"メーカー欄「{str(r.get('observed_maker') or '')[:40]}」"
        if str(r.get("observed_maker") or "") else "メーカー欄は読めません",
        f"導入日の欄「{str(r.get('observed_release') or '')[:30]}」"
        if str(r.get("observed_release") or "") else "導入日の欄は読めません",
        ("本文にDMMの正式名が完全一致であります"
         if r.get("name_in_body") else
         "本文にDMMの正式名は完全一致ではありません"
...
def _gather(name: str, maker: str = "", slug: str = "",
    ...
    if _drop:
        ...
        # ★票（looks）はこのあと got["urls"] から作る★（2026-09-15）＝
        #   取得を先にしたので、外したURLはそもそも同定にかからない。
        if len(got["urls"]) < 2:
            got["problems"].append(
                f"名鑑の個別ページが {len(got['urls'])} 件しか残りません"
                "（取れない・転送されるページを除いた結果）")
            # ★止めない★（2026-09-15・上と同じ理由）
    # ★★同定は、材料を読むのと同じ本文で行う★★（2026-09-15・台帳#675）
    #   ★直す前★＝ここで1回取り、材料を読むときにもう1回取っていた。
    #   ＝分類・2AIへの問い・実際に読む本文が別の写しになり、
    #   ★観測した題やメーカー欄が、記事の材料になった本文と食い違いうる★。
    #   控えを当てる条件に本文の指紋を使うので、ここが同じでないと
    #   2AIが決めた控えが永久に効かない。
    looks = [_mc.lookup(u, name, expected_maker=maker,
                        dmm_identity=_ident_for(u),
                        page=_pages.get(u)) for u in got["urls"]]
...
BLOCKING = (
    # ★★★ここに入れてよいのは3つだけ★★★（2026-09-18・運営者の方針）
    #   ＞ 決まった機種の決められた項目を２AIで調べて突き合わせて結論を出すだけ
    #   ＞ 逆　2AIで不都合が起きれば機械的な要素を入れるんだよ
    #   ＞ 過去に作った機械的なルールは全て無視してもいいくらいだよ
    #   ①外の世界の制約（規約で取れない・そもそも取れない）
    #   ②言うだけでは通らないことの照合（逐語がページに実在するか）
    #   ③公開物が成り立たないこと
    #
    # ★★外したもの（機械が意味を判定していたもの）★★
    #   型式名の食い違い／規格印（L/S）／公式ページと名前が一致しない／
    #   メーカーが名簿にない／メーカー名簿を読めない／メーカーが指定されていない／
    #   登場年月が公式と違う／公式ページに登場年月が無い／
    #   名鑑が見張れていない／公式の場所が名簿にない／
    #   一覧で食い違う／転載の疑い／転載照合を実施できない／
    #   候補が複数で決められない（AMBIGUOUS_CANDIDATES）／
    #   名鑑の索引が不調（CATALOG_UNHEALTHY）／登場年月が新台の範囲外
    #   ★どれも「このページはこの機種か」の判断★＝2AIの仕事。
    #   ★外しても読者は守られる★＝
    #     ・材料に使うページの採否は2AIが決める（答えが無ければ**その晩は使わない**）
    #     ・値ごとの採否は独立2出典（`adoption_basis` / `source_lineage`）
    #     ・根拠の無い値は `build_new_article.require_basis()` が公開を断る
    #     ・材料が1件も無ければ `usable_material` が空になり記事を作らない
    #   ★直す前は、どれか1つが立つと機種まるごと止まっていた★＝
    #   他の出典が健全でも記事にならず、**2か月以上1件も公開できなかった**。
    #
    # ①外の世界の制約
    "公式ページを取得できません",      # そもそも取れない
    "読める状態ではありません",        # メンテ画面など
    "へ転送されました",                # 別のページへ飛ばされた
    "トップページへ転送されました",
    # ②言うだけでは通らないことの照合
    "CONFIRMED_VALUES_UNREADABLE",     # 2AIの確定値の控えが読めない
    MAKER_CACHE_UNREADABLE,            # 2AIが「使わない」と決めた控えが読めない
    # ③公開物が成り立たない
    "既に登録されている疑い",          # 同じ機種を二重に作らない
    # ★★この2つだけは残す★★＝間違えたときに**読者へ別の機種が出る**
    #   （2026-09-18・Codexの助言＝誤同定の被害が大きいものは最後に移す）
    #   ★どちらも今日止まっていた原因ではない★（実測：6件中0件）ので、
    #   残しても進みは変わらない。
    "パチスロのページに見えません",         # パチンコ機をパチスロとして出さない
    "名鑑ごとに型式名が食い違っています",   # 別機種の資料が混ざっている
)
...
def retry_push_first() -> list:
    ...
    if sha and now != sha:
        ...
        if ok_pub:
            # ★★消す直前に、いまもロックを持っているか確かめる★★
            #   （Codexのレビュー15・重大②）ロックは★貸出★なので、
            #   30分止まっている間に別の実行へ移り、その実行が
            #   ★新しい目印を書いている★ことがある。
            #   確かめずに消すと、★別の実行が出す予定のものを消す★。
            ng = lock_still_mine("出してある目印を消す直前")
            if ng:
                return ng
            # ★★読んだときと同じ目印かを、もう一度読んで確かめる★★
            try:
                again = _sj.read_json(PUSH_PENDING, expect=dict)
            except Exception as e:        # noqa: BLE001
                return [f"出せていない公開の目印を読み直せませんでした: {e}"]
            if (again.get("slug") != got.get("slug")
                    or again.get("sha") != got.get("sha")
                    or (again.get("stage") or "") != (got.get("stage") or "")):
                return ["出せていない公開の目印が、確かめている間に"
                        "書き換わりました（消していません）"]
...
def _selftest_body() -> int:
    ...
    try:
        ...
        # ★★同定は、材料を読むのと同じ本文で行う★★（2026-09-15・台帳#675）
        #   ★別々に取ると★＝観測した題やメーカー欄が、記事の材料になった
        #   本文と食い違いうる。控えを当てる条件に本文の指紋を使うので、
        #   ★ここが同じでないと、2AIが決めた控えが永久に効かない★。
        _seen_pages = {"n": 0, "bad": 0}
        _keep_lk675 = _mc.lookup

        def _lookup_watch(u, n, **k):
            _seen_pages["n"] += 1
            if k.get("page") is None:
                _seen_pages["bad"] += 1
            return {"url": u, "identity_ok": True, "model_code": "L1",
                    "reason": "OK", "reason_codes": [], "decision": "ACCEPT",
                    **_MKC(k)}

        _di.find = _two_found
        _mc.lookup = _lookup_watch
        try:
            _gather_counting()
        finally:
            _mc.lookup = _keep_lk675
        t("★★同定には、取ってきた本文をそのまま渡す★★"
          "（★渡さないと、分類した本文と材料にする本文が別物になる★）",
          _seen_pages["n"] == 2 and _seen_pages["bad"] == 0)
        ...
        # ★★2026-09-18：公開を止める側から外した★★（運営者の方針＝既定は2AI）
        #   ★観測としては残す★＝2AIへの問いの材料になる。
        #   ★読者は材料側で守られる★＝そのページを材料に使うかは2AIが決め、
        #   答えが無ければその晩は使わない。
        t("　見張れていない社は、観測として残す（公開は止めない）",
        ...
        # ★★2026-09-18：登場年月は公開を止める理由から外した★★
        #   ★観測としては残す★（2AIが読んで判断する）
        t("　古い機種は、観測として残す（公開は止めない）",
          any("範囲外" in x for x in _old["problems"])
          and not any("範囲外" in x for x in _old["blocked"]))
        _nw._get = lambda u, timeout=20: (
            "<title>L試験機</title><body>2026年9月 登場</body>")
        _bad = run_one("L試験機", "https://m.example/products/slot/zzz/", "nosuch", "2026-09")
        # ★★2026-09-18：メーカー名簿は公開を止める理由から外した★★
        t("　名簿に無いメーカーは、観測として残す（公開は止めない）",
          any("名簿" in x for x in _bad["problems"])
          and not any("名簿" in x for x in _bad["blocked"]))
        ...
        # ★★2026-09-18：メーカー名簿は公開を止める理由から外した★★
        t("　名簿を読めないことは、観測として残す（公開は止めない）",
          any("名簿" in x for x in _brk["problems"])
          and not any("名簿" in x for x in _brk["blocked"]))
        ...
        # ★★2026-09-18：候補が複数で決められないのも2AIへ回す★★
        t("　2票に満たないときの曖昧は、観測として残す（公開は止めない）",
          any("AMBIGUOUS" in x for x in r4b["problems"])
          and not any("AMBIGUOUS" in x for x in r4b["blocked"]))
        ...
        try:
            ...
            t("★★名鑑が2件そろわないことは、公開を止める理由にしない★★"
              ...
              not _blocking([
                  ...
                  "（取れない・転送されるページを除いた結果）"]))
            # ★★2026-09-18：名前の一致・転載の疑いも2AIへ回した★★
            #   ★対照は「読者に別の機種が出るもの」に置き換える★＝
            #   パチンコ機／別機種の資料が混ざる、はいまも止める。
            t("　（対照）読者に別の機種が出るものは、いまも止める",
              _blocking(["パチスロのページに見えません（題・見出しに…）"])
            ...
            try:
                ...
                try:
                    ...
                    #   ★直す前はここで必ず人待ちになり、
                    #     夜の公開が丸ごと止まっていた★
                    _keep_lost2 = list(_LOCK_LOST)
                    _keep_ctx2 = os.environ.pop("UCHIDOKORO_LOCK_CTX", None)
                    _LOCK_LOST.clear()          # ★手前の試験の後片づけ★
                    ...
                    # ★★ロックを失っていたら、出してあっても消さない★★
                    #   （Codexのレビュー15・重大②）
                    #   ★ロックは貸出★なので、止まっている間に別の実行へ移り、
                    #   その実行が新しい目印を書いていることがある。
                    _LOCK_LOST.append("ためしに失った")
                    try:
                        io.open(PUSH_PENDING, "w", encoding="utf-8").write(
                            json.dumps({"slug": "t_resume", "sha": "furuisha",
                                        "stage": "COMMITTED", "parent": "",
                                        "at": "2026/08/29 00:00:00"}))
                        _keep_head4 = globals()["_head"]
                        _keep_pub4 = globals()["_already_published"]
                        try:
                            globals()["_head"] = lambda: "atarasii"
                            globals()["_already_published"] = (
                                lambda sha: (True, ""))
                            ...
                            globals()["_head"] = _keep_head4
                            globals()["_already_published"] = _keep_pub4
                        t("★★ロックを失っていたら、出してあっても"
                          "目印を消さない★★"
                          "（別の実行が書いた目印を消してしまう）",
                          bool(out) and os.path.isfile(PUSH_PENDING))
                    finally:
                        _LOCK_LOST.clear()
                        try:
                            os.remove(PUSH_PENDING)
                    ...
                    #     その実行が新しい目印を書いていることがある★
                    #   確かめずに消すと、別の実行が出す予定のものを消す。
                    _keep_head3 = globals()["_head"]
                    _keep_pub3 = globals()["_already_published"]
                    try:
                        globals()["_head"] = lambda: "atarasii"
                    ...
                    _LOCK_LOST.clear()          # ★手前の試験の状態へ戻す★
                    _LOCK_LOST.extend(_keep_lost2)
                    if _keep_ctx2 is not None:
                        os.environ["UCHIDOKORO_LOCK_CTX"] = _keep_ctx2
                finally:
