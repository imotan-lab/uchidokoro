...
# よって楽天のみ自動監視。
# ★2026-06-26: A8の成果通知は「目標達成通知」（A8公式メール）で対応済み。
#   旧管理画面 ツール→目標達成通知 で「1日単位・1円以上・登録済メール」を設定済み。
#   成果が出た翌日にA8からメールが届く（同一アカウントなのでうちどころ＋わんさかんさい両サイト分カバー）。
#   → A8のスクレイピング自動化は再挑戦不要（公式メールで解決済み）。
A8_ENABLED = False
