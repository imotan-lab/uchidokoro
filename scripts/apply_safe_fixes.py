...
修正ルール:
  R1. checker閾値フォールバックの「機種内慣例」による復元:
      ...
      復元はブロック全体（caution/good/excellent）を慣例値へ揃える。破綻していない
      キーも慣例値と違えば上書きする＝「フォールバック≡byRate[慣例キー]」を機種の
      不変条件として回復する設計（部分修正だと混血値が残り再検知の温床になるため）。
...
def _iter_byrate_blocks(checker):
    """checker配下の「byRateを持つ閾値ブロック」を(パス, ブロック)で列挙。"""
    for mode_key in ("normal", "reset"):
        blk = checker.get(mode_key)
        if isinstance(blk, dict) and isinstance(blk.get("byRate"), dict):
            yield mode_key, blk
    suru = (checker.get("suru") or {}).get("suru")
    if isinstance(suru, list):
        for i, s in enumerate(suru):
            if isinstance(s, dict) and isinstance(s.get("byRate"), dict):
                yield f"suru[{i}]", s
...
def fix_r1_fallback(machine, log):
    ...
    for path, blk in broken:
        ...
        test = dict(blk)
        test.update(newvals)
        if _is_inverted(test):
            log.append(f"    [KEEP] R1 {path}: byRate[{ck}]で復元しても順序破綻が解けない→現状維持")
            continue
        changes = {k: (blk[k], v) for k, v in newvals.items() if blk[k] != v}
        if not changes:
        ...
        for k, (old, new) in changes.items():
            ...
            if state == "replaced" and (old in block_nums_after or old in others_old):
                log.append(f"    [WARN] R1 {path}.note: 旧値{int(old)}が他キーの値とも重複し対応が曖昧→note未変更・手動確認を（数値は修正済み）")
            elif state == "replaced":
                blk["note"] = newnote
                log.append(f"    [FIX] R1 {path}.note: 旧値{int(old)}→{int(new)}に追随置換")
            elif state == "ambiguous":
                log.append(f"    [WARN] R1 {path}.note: 旧値{int(old)}が複数回出現・note文面は手動確認を（数値は修正済み）")
            elif state == "absent" and isinstance(blk.get("note"), str) and \
                    re.search(rf"(?<!\d){int(old)}(?!\d)", blk["note"]):
                log.append(f"    [WARN] R1 {path}.note: 旧値{int(old)}が狙い目表現以外の文脈で出現・note整合を手動確認（数値は修正済み）")
        fixed = True
    return fixed
...
def run(base, slug_filter, apply_mode):
    ...
    for m in machines:
        ...
        try:
            ...
            if r1 or r2:
                n_fix = sum(1 for l in log if "[FIX]" in l)
                total_fixed += n_fix
                if apply_mode:
                    if r1:
                        if machines_rt_safe:
                            ...
                            print(f"    [KEEP] machines.json書き込み不可（非標準フォーマット）→R1修正は未適用")
                            total_unresolved += 1
                    if r2:
                        if detail_raw and _roundtrip_safe(detail_raw):
                            ...
                            print(f"    [KEEP] {dpath.name}は手整形JSONのため書き込み不可→R2修正は未適用（手動対応）")
                            total_unresolved += 1
                else:
