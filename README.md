...
- 全113機種の記事データJSON完成済み（スマート沖スロ ダークハイビは6/22導入予定で天井G数・小役設定差を追記中、とんでもスキルで異世界放浪メシは8月導入予定の先行記事、L ULTRAMAN 最終決戦は7/5導入予定の先行記事、Lからくりサーカス2／スマスロケロット5BT／L南国育ちSPECIALは7/6導入予定の先行記事で解析判明次第更新）
- 交換率4パターン対応済み（Aタイプ除く）
